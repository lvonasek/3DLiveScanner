...
namespace dln {
	...
	public:
		...
		inline const std::vector<TriangleType> &triangulate(std::vector<VertexType> &vertices) {
			...
			for (auto p = begin(vertices); p != end(vertices); p++) {
				...
				_triangles.erase(
						std::remove_if(begin(_triangles), end(_triangles), [](TriangleType &t) {
							return t.isBad;
						}), end(_triangles));
			...
			_triangles.erase(std::remove_if(begin(_triangles), end(_triangles),
											[p1, p2, p3](TriangleType &t) {
												return t.containsVertex(p1) ||
													   t.containsVertex(p2) || t.containsVertex(p3);
											}), end(_triangles));
